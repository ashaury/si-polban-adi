...
<table width="100%" border="1">
  <tr>
    <td width="11%" height="200" bgcolor="#0066CC"><form  method="post" action="laporan_pusat_cetak.php" target="_blank">
      <label>&emsp;&nbsp;
        <select name="upb">
                <option value="" selected="selected" >---Pilih Jurusan---</option>
                <?php
				$link=koneksi_db();
				$sqlbrg="select * from t_upb";
				$resbrg=mysql_query($sqlbrg,$link);
				if($resbrg){
					while($brg=mysql_fetch_array($resbrg)){
                	echo"
					<option value=".$brg['kd_lokasi'].">".$brg['ur_upb']."</option>
					";
					}
				}
				?>
